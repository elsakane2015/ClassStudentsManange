# 智慧校园考勤系统 - 宝塔面板安装教程

本教程适用于在 **宝塔面板 (BT Panel)** 环境下部署智慧校园考勤系统。

---

## 📋 环境要求

| 组件 | 版本要求 | 备注 |
|------|---------|------|
| PHP | 8.2+ | 需安装扩展：fileinfo, bcmath, ctype, json, mbstring, openssl, pdo, tokenizer, xml |
| MySQL | 5.7+ / 8.0+ | 推荐 8.0 |
| Nginx | 1.18+ | 或 Apache 2.4+ |
| Composer | 2.x | PHP 依赖管理 |
| Node.js | 18+ | 用于编译前端资源 |

---

## 🚀 安装步骤

### 第一步：创建站点和数据库

1. 登录宝塔面板
2. 点击 **网站** → **添加站点**
3. 填写信息：
   - **域名**：你的域名（如 `attendance.example.com`）
   - **数据库**：选择 **MySQL**，字符集选择 **utf8mb4**
   - **PHP版本**：选择 **PHP 8.2** 或更高
4. 记录数据库信息（数据库名、用户名、密码）

### 第二步：上传代码

**方式一：Git 克隆（推荐）**
```bash
cd /www/wwwroot/你的站点目录
rm -rf * .*  # 清空目录
git clone https://github.com/你的仓库地址.git .
```

**方式二：上传压缩包**
1. 上传代码压缩包到站点目录
2. 解压后，将文件移动到站点根目录：
```bash
cd /www/wwwroot/你的站点目录
mv 解压目录名/* 解压目录名/.[!.]* ./
rm -rf 解压目录名 压缩包.zip
```

### 第三步：配置环境变量

```bash
cd /www/wwwroot/你的站点目录
cp .env.example .env
nano .env   # 或在宝塔文件管理中编辑
```

**修改以下配置：**
```env
APP_NAME="智慧校园"
APP_ENV=production
APP_DEBUG=false
APP_URL=https://你的域名

DB_CONNECTION=mysql
DB_HOST=127.0.0.1
DB_PORT=3306
DB_DATABASE=你的数据库名
DB_USERNAME=你的数据库用户名
DB_PASSWORD=你的数据库密码
```

### 第四步：安装依赖

```bash
cd /www/wwwroot/你的站点目录

# 安装 PHP 依赖
composer install --no-dev --optimize-autoloader

# 生成应用密钥
php artisan key:generate
```

### 第五步：安装 Node.js 并编译前端

在宝塔 **软件商店** 中安装 **Node.js版本管理器**，然后安装 Node.js 18+。

```bash
# 验证安装
node -v
npm -v

# 安装前端依赖并编译
npm install
npm run build
```

### 第六步：导入数据库

#### 方式一：使用 phpMyAdmin 导入 SQL（推荐）

1. 打开宝塔 → **数据库** → 找到你的数据库 → **管理**（进入 phpMyAdmin）
2. 点击 **导入** 标签
3. 选择文件 `database/schema.sql`，字符集选择 `utf-8`
4. 点击 **执行**

#### 方式二：命令行迁移

```bash
php artisan migrate --force
```

> 如果遇到重复列错误，删除 `database/migrations/` 中重复的迁移文件后重试。

### 第七步：初始化系统数据

#### 7.1 在 phpMyAdmin 中执行 SQL 创建初始数据

打开数据库，点击 **SQL** 标签，执行以下语句：

```sql
-- 1. 创建学校（必须 ID=1）
INSERT INTO schools (id, name, created_at, updated_at) 
VALUES (1, '智慧校园', NOW(), NOW());

-- 2. 创建管理员（邮箱: admin@demo.com，密码: password）
INSERT INTO users (uuid, name, email, password, role, created_at, updated_at) 
VALUES (
    UUID(), 
    'Admin', 
    'admin@demo.com', 
    '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 
    'system_admin', 
    NOW(), 
    NOW()
);
```

#### 7.2 运行 Seeders 填充系统数据

```bash
php artisan db:seed --class=SystemSettingsSeeder --force
php artisan db:seed --class=PermissionSeeder --force
php artisan db:seed --class=LeaveTypeSeeder --force
```

### 第八步：创建 Storage 链接并设置权限

```bash
# 创建 storage 链接
php artisan storage:link

# 清理缓存
php artisan optimize:clear

# 设置目录权限
chown -R www:www /www/wwwroot/你的站点目录
chmod -R 755 /www/wwwroot/你的站点目录
chmod -R 775 storage bootstrap/cache
```

### 第九步：配置网站目录

在宝塔面板中：

1. 点击站点 → **设置** → **网站目录**
2. 将 **运行目录** 设置为 `/public`
3. 保存

### 第十步：配置伪静态

在宝塔面板中：

1. 点击站点 → **设置** → **伪静态**
2. 选择 **laravel** 模板，或手动添加：

```nginx
location / {
    try_files $uri $uri/ /index.php?$query_string;
}
```

3. 保存

### 第十一步：配置 HTTPS（推荐）

1. 点击站点 → **设置** → **SSL**
2. 选择 **Let's Encrypt** 免费证书
3. 开启 **强制HTTPS**

---

## ✅ 验证安装

1. 访问 `https://你的域名`
2. 使用管理员账户登录：
   - **邮箱**：`admin@demo.com`
   - **密码**：`password`
3. **登录后请立即修改密码！**

---

## ⏰ 配置定时任务

在宝塔 **计划任务** 中添加：

| 任务名称 | 执行周期 | 脚本内容 |
|---------|---------|---------|
| Laravel 调度器 | 每分钟 | `cd /www/wwwroot/你的站点目录 && php artisan schedule:run >> /dev/null 2>&1` |

---

## 🔧 常见问题

### 1. 页面显示 500 错误
```bash
# 检查日志
tail -100 storage/logs/laravel.log

# 确保权限正确
chown -R www:www /www/wwwroot/你的站点目录
chmod -R 775 storage bootstrap/cache
```

### 2. 登录密码错误
在 phpMyAdmin 中执行：
```sql
UPDATE users 
SET password = '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi' 
WHERE email = 'admin@demo.com';
```

### 3. 图片上传失败
```bash
# 检查 storage 链接
ls -la public/storage

# 重新创建链接
rm -f public/storage
php artisan storage:link
```

### 4. 样式/脚本加载失败
```bash
# 重新编译前端
npm run build

# 清理缓存
php artisan optimize:clear
```

### 5. 数据库迁移报错"列已存在"
检查 `database/migrations/` 目录中是否有重复的迁移文件（相同功能但不同时间戳），删除重复的文件后重试。

---

## 📦 系统升级

```bash
cd /www/wwwroot/你的站点目录

# 拉取最新代码
git pull origin main

# 更新依赖
composer install --no-dev --optimize-autoloader
npm install
npm run build

# 运行迁移
php artisan migrate --force

# 清理缓存
php artisan optimize:clear
```

---

## 📞 技术支持

如遇到问题，请：
1. 查看 `storage/logs/laravel.log` 中的错误日志
2. 查看宝塔面板的 Nginx/PHP 错误日志
3. 提交 Issue 到项目仓库
