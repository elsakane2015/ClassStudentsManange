...
class EveningStudyController extends Controller
{
    ...
    public function classes(Request $request, AttendancePeriodService $periodService)
    {
        ...
        return response()->json($classes->map(function ($class) use ($sessions, $boardingCounts) {
            return [
                'id' => $class->id,
                'name' => $class->name,
    ...
    public function updateRecords(Request $request, EveningStudySession $eveningStudySession)
    {
        ...
        DB::transaction(function () use ($data, $statuses, $eveningStudySession) {
            foreach ($data['records'] as $item) {
                ...
                $record->update([
                    ...
                    'destination' => $item['destination'] ?? $record->destination,
                    'note' => $item['note'] ?? $record->note,
    ...
    {
        $this->authorizeModifyClass($request, $eveningStudySession->schoolClass);
        abort_unless(in_array($request->user()->role, ['system_admin', 'school_admin', 'department_manager', 'manager'], true), 403);
    ...
    public function history(Request $request)
    {
        ...
        if (!empty($data['date_from'])) $query->whereDate('attendance_date', '>=', $data['date_from']);
        if (!empty($data['date_to'])) $query->whereDate('attendance_date', '<=', $data['date_to']);
        if (!empty($data['class_id'])) $query->where('class_id', $data['class_id']);

        return response()->json($query->latest('attendance_date')->latest('id')->paginate(30));
    }
    ...
    {
        $user = $request->user();
        if (in_array($user->role, ['system_admin', 'school_admin'], true)) return;
        if ($user->role === 'teacher' && $class->teacher_id === $user->id) return;
        if (in_array($user->role, ['department_manager', 'manager'], true)
            && $user->managedDepartments()->whereKey($class->department_id)->exists()) return;
    ...
    private function authorizeModifyClass(Request $request, SchoolClass $class): void
    {
        ...
        if ($user->role === 'duty_teacher'
            && $user->dutyDepartments()->whereKey($class->department_id)->exists()) return;
        if (in_array($user->role, ['department_manager', 'manager'], true)
            && $user->managedDepartments()->whereKey($class->department_id)->exists()) return;
    ...
    private function scopeClasses(Request $request, $query, bool $take = false): void
    {
        ...
        if ($take) abort(403, '只有值班老师或系统管理员可以执行夜自习点名');
