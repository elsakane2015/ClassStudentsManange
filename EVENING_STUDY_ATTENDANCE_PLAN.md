...
夜自习在“系统设置 -> 考勤规则 -> 节次管理”中创建，不在“时段管理”重复配置。
...
4. 学生选择普通节次时，维持现有表单和提交流程。
5. 学生选择 `scene = evening_study` 的节次后：
   - 隐藏普通课程时段细分。
   - 显示关联了系统请假类型的夜自习状态。
   - 先选择请假类型，再选择该类型关联的夜自习状态。
   - “具体去向”必填。
6. 后端通过 `period_id` 读取节次配置并验证：
   - 节次启用。
   - 场景为夜自习。
   - 适用范围允许当前学生。
   - 学生为住宿生。
   - 状态启用且允许学生申请。
   - 状态关联的请假类型与提交类型一致。
7. 创建 `attendance_records`：
   - `scene = evening_study`
   - `counts_in_day_stats = false`
   - `requested_evening_status_id = 申请状态`
   - `evening_study_status_id = null`
   - `destination = 具体去向`
